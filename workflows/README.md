...
仓库自带 `h3-api.template.json`，可以直接覆盖本地 768P 的文字、图片、视频和音频参考生成，并自动匹配当前 ComfyUI 中安装的 H3 模型文件。

如需替换为自己的工作流，在 ComfyUI 中打开已经调通的 H3 工作流，选择“导出（API 格式）”，保存为本目录下的 `h3-api.json`。自定义文件优先于内置模板。
