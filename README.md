...
按提示填写 API 基础地址（通常以 `/v1` 结尾）、模型名称和 API Key。脚本会生成一枚独立的网页访问令牌并自动重启 H3；在“环境部署”页面顶部的“配置服务器助手”入口中输入该令牌即可使用。首次配置完成后，API 地址、模型和密钥可以直接在网页设置里修改。
...
环境部署页的实例终端按 MCSManager 风格固定显示在主工作区，黑色日志区会占据主要高度，命令输入固定在底部；网页只提供受控命令，不是任意 Shell。
